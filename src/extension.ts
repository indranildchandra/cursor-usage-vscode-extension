...
import { TeamMemberSpend, UserUsageResponse } from "./models";
...
async function refreshUsage(context: vscode.ExtensionContext): Promise<void> {
  ...
  try {
    ...
    let maxRequests = userUsage ? (userUsage["gpt-4"].maxRequestUsage || 500) : 500;
    ...
    if (!userUsage) {
      ...
      console.log("[Cursor Usage] User usage API failed - using existing error handling (complete failure)");
      bothApisFailed = true;
    ...
    if (!bothApisFailed) {
      ...
      console.log("[Cursor Usage] Both APIs failed - not updating status bar, letting error handling work");
    }
  } catch (error: any) {
...
export async function testNotification(context: vscode.ExtensionContext) {
  ...
  try {
    ...
    console.log("Test notifications completed - check console for detailed logs");
  } catch (error) {
    console.error("Failed to send test notification:", error);
    vscode.window.showErrorMessage(`Failed to send test notification: ${error}`);
  }
}
...
function getFullTooltipMessage(): string {
  ...
  if (statusBarItem && statusBarItem.tooltip) {
    ...
    while (lines.length > 0 && (lines[lines.length - 1].trim() === '' || lines[lines.length - 1].includes('Click to refresh'))) {
      lines.pop();
    }
...
function sendCursorNotification(message: string) {
  ...
  console.log('Cursor notification sent - will be cleaned from history after 6 hours');
}
...
function trackNotificationForCleanup(message: string) {
  ...
  try {
    ...
    console.log(`Tracking notification - current history count: ${history.length}`);
    ...
    const filteredHistory = history.filter(item => item.timestamp > sixHoursAgo);
    ...
    console.log(`Notification tracked successfully - total in history: ${filteredHistory.length}`);

    // Set up immediate cleanup check
    setTimeout(() => {
      cleanupOldNotifications();
    }, 1000 * 60 * 5); // Check in 5 minutes for immediate feedback
...
function cleanupOldNotifications() {
  ...
  try {
    ...
    console.log(`Cleanup check - current notification count: ${history.length}`);
    ...
    if (filteredHistory.length !== history.length) {
      ...
      console.log(`Cleaned up ${cleanedCount} old notifications from history (older than 6 hours)`);

      // Log some details about what was cleaned
      const oldNotifications = history.filter(item => item.timestamp <= sixHoursAgo);
      if (oldNotifications.length > 0) {
        console.log(`Oldest notification was from ${new Date(oldNotifications[0].timestamp).toLocaleString()}`);
      }
    } else {
...
function initializeNotificationCleanup() {
  try {
    ...
    console.log('Notification cleanup system initialized - will clean notifications older than 6 hours');
...
function sendOSNotification(message: string) {
  ...
  const bulletedMessage = message.split('\n')
  ...
  if (!success) {
    console.log('All OS notification methods failed, but Cursor notification is still working');
  }
}
  ...
  try {
    ...
    for (const testPath of possibleIconPaths) {
      if (fs.existsSync(testPath)) {
        iconPath = testPath;
        console.log('Found icon at:', testPath, '- Size:', fs.statSync(testPath).size, 'bytes - Extension:', path.extname(testPath));
        console.log('Extension context path resolution: __dirname =', __dirname);
    ...
    if (!iconPath) {
      console.log('No icon found in any of the expected paths. Will send notification without icon.');
    }
    ...
    notifier.notify(notificationOptions, (err: any, response: any, metadata: any) => {
      if (err) {
        ...
        if (err.code === 'ENOENT' && err.path && err.path.includes('terminal-notifier')) {
          console.log('OS notification sent via node-notifier (terminal-notifier fallback not available, but notification sent)');
        } else {
          console.log('OS notification sent via node-notifier with Cursor icon');
        }
        ...
        if (metadata && metadata.activationType === 'clicked') {
          ...
          if (metadata.activationValue === 'Open Cursor') {
            ...
            require('child_process').exec('open -a "Cursor"', (error: any) => {
              if (error) {
...
function sendWithNotificationCenterCheck(message: string): boolean {
  try {
    ...
    console.log('Executing notification center command with Cursor app:', notificationCommand);

    exec(notificationCommand, (notifError: any, notifStdout: any, notifStderr: any) => {
      if (notifError) {
        console.log('Notification center notification failed:', notifError.message);
        ...
        exec(fallbackCommand, (fbError: any, fbStdout: any, fbStderr: any) => {
          if (fbError) {
            console.log('Fallback notification center also failed:', fbError.message);
          } else {
            console.log('OS notification sent via fallback Notification Center');
          }
        });
      } else {
        console.log('OS notification sent via Notification Center with Cursor app');
      }
    });
...
function sendWithTerminalNotifier(message: string): boolean {
  try {
    ...
    console.log('Executing macOS notification command with Cursor app:', command);
    ...
    exec(command, (error: any, stdout: any, stderr: any) => {
      if (error) {
        ...
        exec(altCommand, (altError: any, altStdout: any, altStderr: any) => {
          if (altError) {
            console.log('Alternative AppleScript also failed:', altError.message);
          } else {
            console.log('OS notification sent via alternative AppleScript');
          }
        });
      } else {
        console.log('OS notification sent via macOS AppleScript with Cursor app');
      }
    });
